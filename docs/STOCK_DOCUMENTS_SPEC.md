...
- **Инвентаризация (INV)** не меняет `stock` при проведении: создаёт черновики WOF/PST по дельте (факт − учёт); остатки меняют уже они. `GET /api/documents/inventory/stock-preview?warehouseId=` — предзаполнение плана.
