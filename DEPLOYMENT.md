...
Техзаметка 2026-08-26: числовые поля фронта унифицированы через `DecimalInput` (приёмка/инвентаризация/производство/CRUD); доп. шагов деплоя не требуется, достаточно стандартной сборки фронта.

На ВМ не запускать `npm run seed` вручную. Очистка/демо/рецептуры ВИЛАР/слепки — **Администрирование → Данные и резервные копии** (только Admin). Загрузка рецептур затирает справочники и документы.

Публичная ВМ: оценка рисков и план — `docs/SECURITY_PUBLIC_VM.md`.
ПДн / правовая модель пилота — `docs/LEGAL_PDN.md`.  
...
На пилоте после выкладки кода: **Администрирование → Данные и резервные копии → Загрузить рецептуры ВИЛАР** (подтверждение `RECIPES`). Это очищает оперативные данные и заполняет справочники из `customer_recipes.json`. Не использовать `npm run seed` на живой базе.
