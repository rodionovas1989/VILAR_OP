...
## [2026-09-07] ingest | Prod FB tickets lot IDN display

FB-2026-09-01-00001: отчёт запасов — лист дерева = идентификатор; колонка «Партия производителя».  
...
## [2026-09-08] decision | D011 модели учёта и sequence партии

Настройки системы → модели учёта (Стандартная / Внутреннее производство). FEFO/FIFO: expiry → productionDate → productionSequence. Шаблон номера — калькулятор блоков, разбор rtl. Страницы: accounting-models, D011, fefo-fifo-picking.

## [2026-09-08] feature | Форма модели учёта: тогл+подсказка, шаблон в окне

Карточка: `LabeledToggle` + переиспользуемый `HintButton` («?» → модалка). Шаблон партии — пример номера и карандаш; блоки/порядок во вложенном окне. Modal — portal в `document.body`.

## [2026-09-08] change | Настройки системы внизу меню

Группа NAV / RBAC «Настройки системы» после «Администрирование».

## [2026-09-08] query | Политика смешения партий (G1)

Не в коде. Политика на модели учёта (`single` / `sameManufacturer` / `validated`); заказ — N строк на позицию спеки. Не путать с D010 (одна партия, много складов). Страница: analyses/lot-mixing-policy.

## [2026-09-08] feature | Смешение партий одного производителя

Вкладки модели: Основное / Шаблоны / Смешение. Тогл `mixSameManufacturer` → N строк в подборе, RES, факте, PRI. Разные производители и справочник качества «Валидированное смешение партий» (конкретные номера) — не в коде (lot-mixing-policy).

## [2026-09-08] change | Колонка «Свободно» в подборе

Отображает остаток для строки с учётом уже предложенного в заказах выше (алгоритм не менялся).

## [2026-09-08] change | Лента «Что нового»: ширина и смысл

Лента на всю ширину рабочей области (убран `max-width: 52rem`). Пункты CHANGELOG: не только «что», но зачем / бизнес-логика; парсер принимает абзацы после `- [kind]`. Страница: changelog-feed.

## [2026-09-08] ops | Deploy main на пилот

Выкат `main`@06c314a на https://vilar-prod.ru: модели учёта, смешение одного производителя, расширенная лента «Что нового». Бэкап sqlite на ВМ перед pull. Не seed.
