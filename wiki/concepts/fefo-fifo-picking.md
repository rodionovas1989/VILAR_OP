...
- **Пакетный подбор** нескольких заказов: общий виртуальный расход по ключу партия×склад (`suggestPicksBulk`). Порядок заказов в пакете — по sequence серии (если у продукта модель с шаблоном), иначе как выбраны. Если FEFO-партия «съедена» предыдущим заказом в пакете, следующий заказ получает **следующую** подходящую партию (GMP: без смешивания партий внутри одной серии).
