...
export default function AccountingModelForm({ editing, setEditing }: Props) {
  ...
  return (
    <div className="accounting-model-form spec-detail-tabs">
      <div className="tabs spec-inner-tabs">
        ...
        <button type="button" className={tab === 'mix' ? 'active' : ''} onClick={() => setTab('mix')}>
          Смешение партий
        </button>
      </div>
      ...
      {tab === 'mix' ? (
        <LabeledToggle
          checked={Boolean(editing.mixSameManufacturer)}
          onCheckedChange={(on) => setField('mixSameManufacturer', on)}
          label="Смешение партий одного производителя"
          hint={
            <>
              <p>
                По умолчанию в одной серии на компонент — одна партия (GMP). Если включить, подбор может взять
                несколько партий <strong>одного производителя</strong>, когда одной не хватает на серию.
              </p>
              <p>
                В заказе появятся несколько строк сырья. Смешение разных производителей здесь не включается:
                для этого позже — справочник качества «Валидированное смешение партий».
              </p>
            </>
          }
        />
